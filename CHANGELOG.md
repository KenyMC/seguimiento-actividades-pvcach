...
## [2026-09-18] - Mejoras en Lógica de Detalles de Riesgo Sanitario (Excedencias de LMP)
### Añadido / Modificado
- **Nuevas Columnas de Detalle y Trazabilidad:** Se incorporaron las columnas "Detalle Fecha del parametro que excede LMP" y "Ubicación Muestreo del parametro que excede LMP" en la pestaña de Riesgos Sanitarios. Esto permite que la tabla web y el Excel vinculen cada violación de calidad de agua con la fecha exacta y la ubicación específica (Reservorio, Red de distribución, etc.) donde se tomó la muestra.
- **Inclusión de Parámetros Bacteriológicos:** Anteriormente, las columnas de texto descriptivo omitían las fallas por contaminación fecal. Ahora, el código evalúa e incluye explícitamente las seis métricas bacteriológicas (Coliformes Fecales, Coliformes Totales y E. Coli, en NMP y UFC) en las columnas de detalle si superan su límite permisible (0 para UFC y 1.8 para NMP).

### Corregido / Mejorado
- **Registro Exhaustivo de Múltiples Excedencias:** Se solucionó un bug lógico de agrupación por SAP que provocaba el ocultamiento de violaciones secundarias (ej. una muestra fallida en Red de Distribución era descartada e invisible si la muestra del Reservorio tenía un valor numérico mayor). Se reescribió el algoritmo para guardar un historial global (`map[id].exceeds`) que recolecta *todas* las instancias donde un parámetro excede el LMP durante el periodo filtrado, independientemente de si es el valor máximo absoluto o no.
- **Ordenamiento Cronológico Inteligente (Más Reciente a Más Antiguo):** Para mejorar la legibilidad de la lista de violaciones separada por comas, se integró un algoritmo que ordena las excedencias por fecha de forma descendente (parseando el formato DD-MM-YYYY). De este modo, los parámetros que fallaron más recientemente aparecen primero. En caso de existir fallas simultáneas el mismo día, el sistema desempatará ordenándolos según la jerarquía y orden oficial de las columnas (Bacterias primero, seguido de parámetros fisicoquímicos).

## [2026-08-29] - Corrección de Agrupación Dinámica en Exportación Excel (FED AI-01.01)
### Corregido / Optimizado
- **Sincronización de Conteo CCPP (Excel vs Web):** Se solucionó un desfase estadístico donde el archivo Excel exportado de la sección FED (AI-01.01) omitía sumar correctamente centros poblados que lograban el cumplimiento a través de la combinación de múltiples Sistemas de Agua (SAPs). Anteriormente, el algoritmo de exportación evaluaba únicamente al *primer* SAP de la lista. Ahora, la exportación captura dinámicamente las columnas condicionales generadas en formato mensual (ej. `Amb. Jun E. Bueno`, `Cumple Salud Junio`, etc.) y aplica una evaluación de compuerta OR inteligente. Si al menos un SAP cumple las condiciones dentro del mismo CCPP, la exportación agrupa el CCPP como "Cumple", emparejando el total matemático al 100% con la tabla visualizada en la aplicación web.
- **Soporte Dinámico para Hoja Consolidado:** Se adaptó la hoja "Consolidado" dentro del mismo reporte Excel para que sus sumatorias busquen las columnas dinámicas correctas según el periodo seleccionado (ej. `Cumple >=5 Meses SALUD` en lugar del antiguo nombre estático `Cumple Salud`), evitando que queden en 0 tras la actualización del formato post Junio-2026.

## [2026-08-25] - Reestructuración de Lógica FED, Filtros Mensuales y Cruce Transversal de SAPs
### Añadido / Modificado
- **Extracción Dinámica de Id. SAP por Separador Pipe (`|`):** Se incorporó una regla inteligente en `matchSapIds` (aplicable a las pestañas *Vigilancia* y *Vigilancia FED*) que permite detectar cuando un sistema viene con formato compuesto en la columna "Nombre SAP" (ej. `20827 | CCOÑIPUCYO`). Si la columna "Id. SAP" está ausente (como en el caso de la hoja RIESGOS), el sistema dividirá automáticamente esta cadena por el carácter pipe (`|`) y asignará la primera parte como `Id. SAP` y la segunda como `Nombre SAP`. Esto restablece y asegura los cruces de datos entre bases de datos secundarias que carecen de identificadores limpios o de columna Ubigeo.
- **Filtro Estricto Mensual para Pestaña "Insp. Sanitaria":** Se modificó la lógica de la pestaña individual de *Insp. Sanitaria* (dentro de Actividades SAP). Aunque la tabla sigue manteniendo visualmente las columnas agrupadas por semestres (ej. "1ra Inspección 2026"), ahora el algoritmo exige que la **Fecha de inspección** del registro caiga *exactamente* dentro de los meses seleccionados en el filtro global superior. Esto permite que, si el usuario filtra de "Jun 2026 a Jun 2026", la tabla oculte las inspecciones de Enero a Mayo y responda al instante mostrando únicamente los registros efectuados en Junio.
- **Redefinición Técnica de Semestres para Vigilancia FED:** A fin de alinearse estrictamente con la ficha técnica del Fondo de Estímulo al Desempeño (FED), la lógica de validación de semestres en la pestaña *Vigilancia (FED)* fue alterada. Ahora, el **Primer Semestre** abarca exclusivamente de **Enero a Mayo** (`<= 5`), mientras que el **Segundo Semestre** abarca de **Junio a Diciembre**. Esto afecta la contabilidad de actividades como Caracterización, las cuales exigen un registro por semestre técnico.
- **Filtro Estricto Mensual para Inspección Sanitaria:** Se reescribió la validación de Inspecciones dentro de la pestaña *Vigilancia (FED)*. En lugar de aprobar inspecciones de forma relajada por correspondencia de "semestres", el sistema ahora valida de forma estricta que la **Fecha de inspección** (extrayendo el mes nativo directamente de dicha columna en caso de que la columna "Mes" esté vacía o sea inválida) caiga *exactamente* dentro de los meses seleccionados en el filtro global de fechas de la interfaz superior. Por ejemplo, si se filtra de Enero a Abril, solo sumará las inspecciones ocurridas en ese rango temporal.
- **Exigencia Específica de 5P (3 en Red / 1 en Reservorio):** Para que un mes califique como "Completo" en la columna Monitoreo 5P dentro de *Vigilancia (FED)*, se introdujo la validación posicional: Independientemente de si se tomaron múltiples muestras (ej. 9 puntos adicionales incompletos), el mes es válido si, de forma agrupada en el mismo bloque temporal, se logra evidenciar que al menos **3 puntos ubicados en Red de distribución** (o piletas/viviendas) y **1 punto ubicado en Reservorio** cuentan con los 5 parámetros válidos de cloro, turbiedad, pH, conductividad y temperatura. Las exigencias de meses mínimos requeridos (tanto para 5P como Riesgos) ahora también se adaptan dinámicamente al tamaño del rango de fechas que elija el usuario.
- **Restauración del Diccionario Transversal de SAPs (`sapToCcppId`):** Se resolvió un error inducido que provocaba pantallas en blanco o "0 REG" en Inspección y Riesgos de la vista FED al borrar accidentalmente la generación del diccionario de cruce en un refactor previo. La recuperación de esta estructura garantiza que los ID numéricos en hojas de datos carentes de Ubigeo vuelvan a emparejar con los registros del padrón MEF (MAIN).

## [2026-08-10] - Correcciones de Filtro "Registro a tiempo"

- **Re-renderizado Automático:** Se agregó la función `handleRegistroTiempoChange` para que al activar/desactivar el filtro se borren correctamente las cachés (`APP_STATE.fedCache` y `APP_STATE.ptCache`) y se redibuje la vista al instante mediante `processActiveData()`. Esto soluciona el problema de que el filtro "no hacía nada" al probarse en la pestaña AI-01.01 u otras.
- **Mejora en Detección de Fechas:** Se optimizó la expresión regular que lee la columna "Fecha Finalizado" para que sea capaz de interpretar formatos de fecha variables como `DD-MM-YYYY`, `DD/MM/YYYY HH:mm` (sin segundos) o `D/M/YYYY` que vienen del export de la matriz. Al fallar el parseo antiguo, la función `isRegisteredOnTime` retornaba siempre falso y los indicadores caían a 0. Ahora el procesamiento es robusto, cuenta correctamente las muestras finalizadas a tiempo y no se rompe con formatos parciales.
...
- **Corrección en Cálculo de Sistemas Faltantes frente a Meta MEF (`renderConsolidatedAndChart`):** Se ajustó la fórmula de la tabla resumen para el Ámbito MEF. Ante la existencia de sistemas duplicados en el padrón `MEF_UB`, el código ahora preserva la cifra original de la Meta MEF y calcula la columna "Sin Monitoreo" basada estrictamente en los sistemas *únicos* que realmente faltan. Los registros duplicados son absorbidos por la columna "Completo", logrando que los sistemas faltantes cuadren exactamente (a 1 o 0, según corresponda) sin generar sistemas "fantasma".
- **Visualización de Incompletos en "Sistemas No Monitoreados" de Riesgos:** El modal de sistemas no monitoreados para la pestaña de Riesgos ahora incluye también a los sistemas que tienen estado "Incompleto" (es decir, aquellos a los que les falta subir el Informe, el Cargo, o ambos). Anteriormente, si un sistema tenía aunque sea un documento, se le consideraba monitoreado y se excluía del modal. Ahora, debe tener ambos documentos `aprobados` (o validados por regularización) para no aparecer en la lista de faltantes.
- **Regla de Cumplimiento en Monitoreo 5P para Puntos Adicionales:** Se ajustó la regla de evaluación de "Completo" en la pestaña de Monitoreo 5P. Anteriormente, si se evaluaban puntos adicionales (ej. durante una inspección se miden 9 puntos) y alguno quedaba incompleto, se invalidaba todo el mes. Ahora, el sistema evaluará el mes como "Completo" siempre y cuando se haya cumplido con el registro de los 5 parámetros en al menos **3 puntos de Red de distribución** (o vivienda/pileta) y **1 punto de Reservorio**, ignorando si hay puntos extras incompletos.
- **Exportación a Excel Nativo en Modal de No Monitoreados:** Se actualizó la función `exportSanitariaUnmonitoredExcel` para generar un archivo Excel nativo (`.xlsx`) utilizando la librería SheetJS, en lugar de descargar un archivo de texto `.csv`.
- **Nueva Pestaña "Vigilancia (FED)":** Se añadió una nueva pestaña en la sección SAP que consolida el cumplimiento simultáneo de las 4 actividades exigidas por el FED (Inspección, Caracterización, Monitoreo 5P y Riesgos). Las exigencias (número de meses mínimos para 5P y Riesgos) se adaptan dinámicamente según la cantidad de meses seleccionados en el filtro global superior. La tabla resumen consolida los sistemas que logran la "Vigilancia Completa" y suma el total de inspecciones y caracterizaciones realizadas.
- **Regla Estricta del 75% en Pestaña Nivel de Riesgo:** Se modificó la lógica de evaluación de los parámetros básicos (Cloro, Turbiedad, pH, Conductividad, Temperatura). Ahora se exige estrictamente que **mayor o igual al 75%** de las muestras del mes cumplan con los límites permisibles para que el parámetro global sea calificado como "Cumple". Anteriormente, la evaluación era más permisiva y solo requería que 1 sola muestra cumpliera.
- **Nueva Pestaña "Resultado FED" en Exportación Excel (AI-01.01):** Se añadió una nueva pestaña al archivo Excel exportado desde la sección FED (AI-01.01). Esta pestaña mantiene las mismas columnas que la pestaña "Evaluación", pero presenta valores binarios: 1 para "Cumple" y 0 para "No Cumple". Adicionalmente, para las columnas de los meses (ej. *Jul (Cl>=0.5)* o *Turb<=5*), aplica la regla de que si el valor mensual acumulado es menor a 3, se marca directamente con un 0, y caso contrario con un 1.
